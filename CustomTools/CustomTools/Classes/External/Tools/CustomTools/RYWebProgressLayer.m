...
#import "RYWebProgressLayer.h"
#import "NSTimer+addition.h"


static NSTimeInterval const kFastTimeInterval = 0.03;

@implementation RYWebProgressLayer   {
    CAShapeLayer *_layer;
...
+ (instancetype)layerWithFrame:(CGRect)frame {
    RYWebProgressLayer *layer = [self new];
    layer.frame = frame;
...
- (void)initialize {
    ...
    self.lineWidth = 2;
    self.strokeColor = [UIColor colorWithHexString:@"#06c1ae"].CGColor;
    __weak typeof(self) weakSelf = self;
    _timer = [NSTimer ry_scheduledTimerWithTimeInterval:kFastTimeInterval repeats:YES block:^(NSTimer *timer) {
